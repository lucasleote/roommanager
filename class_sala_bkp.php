...
#--------------------------------------C L A S S E   S A L A--------------------------------------------------------#
	Class Sala
	{
		...
		{ 
			...
			$select = 'SELECT salas.id, salas.nome, tipo_sala.descricao AS saladesc, salas.capacidade, blocos.descricao AS blocodesc, salarecurso.id_recurso, recursos.nome AS recurso, salas.obs, DATE_FORMAT(salas.data,"%d/%m/%y") AS data, DATE_FORMAT(salas.hora,"%H:%i") AS hora 
			FROM salas,tipo_sala,blocos,salarecurso,recursos 
			WHERE salas.id_tipo = tipo_sala.id AND salas.id_bloco = blocos.id AND salas.id = salarecurso.id_sala AND recursos.id = salarecurso.id_recurso 
			ORDER BY id DESC';
